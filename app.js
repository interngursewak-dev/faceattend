  ...
  STATE.addDetectionLoop = setInterval(function() {
    ...
    detectFace(DOM.videoAdd, DOM.overlayAdd, null, DOM.addFaceInd)
      .then(function(desc) {
        ...
        // Enable capture button when face is visible (and not already captured)
        if (desc && !STATE.capturedDescriptor) {
...
function handleCaptureFace() {
  // Try current descriptor first, or do a fresh detection
  var desc = STATE.addCurrentDescriptor;

  if (!desc) {
    DOM.captureStatus.textContent = 'Detecting face…';
    detectFace(DOM.videoAdd, DOM.overlayAdd, null, DOM.addFaceInd)
      .then(function(freshDesc) {
        if (!freshDesc) {
          showToast('No face detected — look at camera', 'error');
          DOM.captureStatus.textContent = 'No face detected — try again';
          DOM.captureStatus.className = 'capture-status err';
          return;
        }
        saveCapturedFace(freshDesc);
      });
    return;
  }
